...
from .utils_gmm import calculate_matmul, calculate_matmul_n_times
...
class GaussianMixture(torch.nn.Module):
    ...
    def __init__(self, n_components, n_features, covariance_type="full", eps=1.e-6, init_params="kmeans", mu_init=None,
        ...
        self.reg_covar = torch.Tensor([1.e-6]).to(self.dtype)
    ...
    def _init_params(self):
        if self.mu_init is not None:
            ...
            self.mu = torch.nn.Parameter(
                torch.randn(1, self.n_components, self.n_features).to(self.dtype), requires_grad=False)
        ...
        if self.covariance_type == "diag":
            ...
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(
                    torch.eye(self.n_features).reshape(
                        1, 1, self.n_features, self.n_features).repeat(1, self.n_components, 1, 1).to(self.dtype),
                    requires_grad=False
                )

        # (1, k, 1)
        self.pi = torch.nn.Parameter(torch.Tensor(1, self.n_components, 1).to(self.dtype), requires_grad=False).fill_(
            1. / self.n_components)
    ...
    def fit(self, x, delta=1e-3, n_iter=100, warm_start=False):
        ...
        while (i <= n_iter) and (j >= delta):
            print("Iteration %i / %i: log-likelihood = %.3f" % (i, n_iter, self.log_likelihood))
    ...
    def sample(self, n):
        ...
        for k in np.arange(self.n_components)[counts.numpy() > 0]:
            if self.covariance_type == "diag":
    ...
    def _estimate_log_prob(self, x):
        ...
        if self.covariance_type == "full":
            ...
            precision = torch.inverse(var).to(x.device)
            d = x.shape[-1]
    ...
    def _e_step(self, x):
        ...
        """
        x = self.check_size(x)
        weighted_log_prob = self._estimate_log_prob(x) + torch.log(self.pi.to(x.device))
    ...
    def _m_step(self, x, log_resp):
        ...
        if self.covariance_type == "full":
            ...
            # shape [1, n_components, n_features, n_features]

            diff = x - mu  # Shape: (n_samples, n_components, n_features)
            diff = diff.transpose(0, 1)
            weighted_diff = resp.transpose(0, 1) * diff  # Shape: (n_components, n_samples, n_features)
            covariances = torch.matmul(weighted_diff.transpose(1, 2), diff)
            covariances = covariances / pi.view(-1, 1, 1)
            covariances += torch.eye(covariances.size(-1), device=x.device).unsqueeze(0) * self.reg_covar.to(x.device)
            var = covariances.unsqueeze(0)
            # breakpoint()
            pass
    ...
    def numpy_em(self, x):
        weighted_log_prob = self._estimate_log_prob(x).numpy() + torch.log(self.pi).numpy()
        # x = x.squeeze(dim=1).numpy()
        log_prob_norm = logsumexp(weighted_log_prob, axis=1)
        with np.errstate(under="ignore"):
            # ignore underflow
            log_resp = weighted_log_prob - log_prob_norm[:, np.newaxis]
        log_prob_norm, log_resp = np.mean(log_prob_norm), np.squeeze(log_resp, axis=2)  # (1000, 2, 1)
        x = x.squeeze(dim=1).numpy()
        # print('numpy_e', log_prob_norm, log_resp[:3])
        # print(x.shape, log_prob_norm, log_resp.shape)

        # torch.Size([1000, 1, 2]) -2.85048796358561 (1000, 2, 1)
        # (1000, 2) -2.832810182359336 (1000, 2)
        ...
        for k in range(n_components):
            ...
            covariances[k].flat[:: n_features + 1] += self.reg_covar.numpy()
            print(f"nk: {nk}, covariances.shape: {covariances[k].shape}")
            print(f"k: {k} diff: {diff[:2, :2]}")
            print(f"nk: {nk}, \n\ncovariances: {covariances[k]}")
        # nk, means, covariances
        weights_, means_, covariances_ = nk, means, covariances
        weights_ /= weights_.sum()
        n_components, n_features, _ = covariances.shape
        precisions_cholesky_ = np.empty((n_components, n_features, n_features))
        for k, covariance in enumerate(covariances):
            try:
                cov_chol = linalg.cholesky(covariance, lower=True)
            except linalg.LinAlgError:
                raise ValueError('Fitting the mixture model failed because some components have')
            precisions_cholesky_[k] = linalg.solve_triangular(
                cov_chol, np.eye(n_features), lower=True
            ).T
        ...
        nk_t = torch.from_numpy(nk)

        diff_t = x_t.unsqueeze(0) - means_t.unsqueeze(1)  # Shape: (n_components, n_samples, n_features)
        weighted_diff_t = resp_t.unsqueeze(2).transpose(0, 1) * diff_t  # Shape: (n_components, n_samples, n_features)
        covariances = torch.matmul(weighted_diff_t.transpose(1, 2), diff_t)
        covariances = covariances / nk_t.view(-1, 1, 1)
        covariances += torch.eye(covariances.size(-1), device=x_t.device).unsqueeze(0) * self.reg_covar
        breakpoint()
    ...
    def __em(self, x):
        ...
        # print('torch_e_step', log_prob_norm, log_resp[:3])

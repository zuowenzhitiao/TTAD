...
def make_patches_for_pcl_triple(pcl_clean, pcl_noisy1, pcl_noisy2, patch_size, num_patches, ratio):
    """
    Args:
        pcl_clean:  The clean point cloud, (N, 3).
        pcl_noisy1:  The first noisy point cloud, (rN, 3).
        pcl_noisy2:  The second noisy point cloud, (rN, 3).
        patch_size:   Patch size M.
        num_patches:  Number of patches P.
        ratio:    Ratio r.
    Returns:
    ...
    N = pcl_clean.size(0)
    seed_idx = torch.randperm(N)[:num_patches]   # (P, )
    seed_pnts = pcl_clean[seed_idx].unsqueeze(0)   # (1, P, 3)
    _, _, pat_clean = pytorch3d.ops.knn_points(seed_pnts, pcl_clean.unsqueeze(0), K=patch_size, return_nn=True)
    pat_clean = pat_clean[0]    # (P, M, 3)
    _, _, pat_noisy1 = pytorch3d.ops.knn_points(seed_pnts, pcl_noisy1.unsqueeze(0), K=int(ratio*patch_size), return_nn=True)
    pat_noisy1 = pat_noisy1[0]
    _, _, pat_noisy2 = pytorch3d.ops.knn_points(seed_pnts, pcl_noisy2.unsqueeze(0), K=int(ratio*patch_size), return_nn=True)
    pat_noisy2 = pat_noisy2[0]
    return pat_clean, pat_noisy1, pat_noisy2
...
class TripledPatchDataset(Dataset):
    ...
    def make_patches(self):
        for dataset in tqdm(self.datasets, desc='MakePatch'):
            for data in tqdm(dataset):
                pat_clean, pat_noisy1, pat_noisy2 = make_patches_for_pcl_triple(
                    data['pcl_clean'],
                    data['pcl_noisy1'],
                    data['pcl_noisy2'],
                    patch_size=self.patch_size,
                ...
                for i in range(pat_clean.size(0)):
                    self.patches.append((pat_clean[i], pat_noisy1[i], pat_noisy2[i]))
    ...
    def __getitem__(self, idx):
        if self.on_the_fly:
            ...
            data = {
                'pcl_clean': pat_clean[0],
                'pcl_noisy1': pat_noisy1[0],
                'pcl_noisy2': pat_noisy2[0]
            }
        else:
            data = {
                'pcl_clean': self.patches[idx][0].clone(),
                'pcl_noisy1': self.patches[idx][1].clone(),
                'pcl_noisy2': self.patches[idx][2].clone(),
            }
        if self.transform is not None:
            data = self.transform(data)
        return data